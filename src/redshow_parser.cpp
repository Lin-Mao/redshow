...
void output_instructions(redshow::SymbolVector &symbol_vector, redshow::InstructionGraph &inst_graph) {
  std::sort(symbol_vector.begin(), symbol_vector.end(),
    [](const redshow::Symbol &l, const redshow::Symbol &r) {
  ...
  for (auto iter = inst_graph.nodes_begin(); iter != inst_graph.nodes_end(); ++iter) {
    ...
    if (inst.access_kind.get() != NULL) {
      ...
      if (iter != symbol_vector.begin()) {
        --iter;
        std::cout << "FUNC: " << iter->index << ", PC: 0x" << std::hex <<
          inst.pc - iter->offset << std::dec <<
          ", ACCESS_KIND: " << inst.access_kind->to_string() << std::endl;
      }
    }
  }
}
