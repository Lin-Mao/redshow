...
std::string DataDependency::get_data_flow_edge_type(EdgeType type) {
  static std::string data_flow_edge_type[] = {"WRITE", "READ", "SINK"};
  return data_flow_edge_type[type];
}

void DataDependency::init() {
...
void DataDependency::kernel_op_callback(std::shared_ptr<Kernel> op) {
  if (_trace.get() == NULL) {
  ...
  for (auto &mem_iter : _trace->write_memory) {
    ...
    if (memory->op_id > REDSHOW_MEMORY_HOST) {
      ...
      auto device_end_max = std::prev(mem_iter.second.end())->end; // @Lin-Mao: MemoryRange is sorted by start?
      auto device_range_len = device_end_max - device_start_min;
      auto overwrite_ratio = static_cast<double>(overwrite) / memory->len;  // @Lin-Mao: overwirte means accessed interval sum?
      auto device_range_ratio = static_cast<double>(device_range_len) / memory->len; // @Lin-Mao: has untouched interval?
      ...
      if (overwrite_ratio < _FRAGMENT_RATIO_LIMIT && memory->len > _FRAGMENT_SIZE_LIMIT &&
        ...
        dtoh(host_cache + device_start_min - device, device, device_range_len); // @Lin-Mao: ?
...
void DataDependency::memory_op_callback(std::shared_ptr<Memory> op) {
...
void DataDependency::memset_op_callback(std::shared_ptr<Memset> op) {
...
void DataDependency::memcpy_op_callback(std::shared_ptr<Memcpy> op) {
...
void DataDependency::op_callback(OperationPtr op, bool is_submemory /* default = false */) {
...
void DataDependency::analysis_begin(u32 cpu_thread, i32 kernel_id, u64 host_op_id, u32 stream_id,
  ...
  if (!this->_kernel_trace[cpu_thread].has(kernel_id)) {
    auto trace = std::make_shared<DataDependencyTrace>();
  ...
  _trace = std::dynamic_pointer_cast<DataDependencyTrace>(this->_kernel_trace[cpu_thread][kernel_id]);

  unlock();
}

void DataDependency::analysis_end(u32 cpu_thread, i32 kernel_id) {}

void DataDependency::block_enter(const ThreadId &thread_id) {
  // No operation
}

void DataDependency::block_exit(const ThreadId &thread_id) {
  // No operation
}

void DataDependency::merge_memory_range(Set<MemoryRange> &memory, const MemoryRange &memory_range) {
...
void DataDependency::unit_access(i32 kernel_id, u64 host_op_id, const ThreadId &thread_id, const AccessKind &access_kind,
...
void DataDependency::flush_thread(u32 cpu_thread, const std::string &output_dir,
                            const LockableMap<u32, Cubin> &cubins,
                            redshow_record_data_callback_func record_data_callback) {}

void DataDependency::flush(const std::string &output_dir, const LockableMap<u32, Cubin> &cubins,
                     redshow_record_data_callback_func record_data_callback) {
...
void DataDependency::link_ctx_node(i32 src_ctx_id, i32 dst_ctx_id, i32 mem_ctx_id, EdgeType type) {
  auto edge_index = EdgeIndex(src_ctx_id, dst_ctx_id, mem_ctx_id, type);
  _graph.add_edge(std::move(edge_index), type);
}

void DataDependency::link_op_node(u64 op_id, i32 ctx_id, i32 mem_ctx_id) {
...
void DataDependency::update_op_node(u64 op_id, i32 ctx_id) {
...
void DataDependency::update_op_metrics(u64 op_id, i32 ctx_id, i32 mem_ctx_id, u64 redundancy,
...
void DataDependency::update_edge_metrics(i32 src_ctx_id, i32 dst_ctx_id, i32 mem_ctx_id, u64 redundancy,
                                   u64 overwrite, u64 count, EdgeType type) {
  // Update current edge's property
  auto edge_index = EdgeIndex(src_ctx_id, dst_ctx_id, mem_ctx_id, type);
...
void DataDependency::analyze_duplicate(Map<i32, Map<i32, bool>> &duplicate) {
...
void DataDependency::dump(const std::string &output_dir, const Map<i32, Map<i32, bool>> &duplicate) {
