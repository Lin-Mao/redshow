#ifndef REDSHOW_ANALYSIS_DATA_DEPENDENCY_H
#define REDSHOW_ANALYSIS_DATA_DEPENDENCY_H
...
class DataDependency final : public Analysis {
 public:
  DataDependency() : Analysis(REDSHOW_ANALYSIS_DATA_DEPENDENCY) { init(); }

  virtual ~DataDependency() = default;

  // Coarse-grained
  virtual void op_callback(OperationPtr operation, bool is_submemory = false);

  // Fine-grained
  virtual void analysis_begin(u32 cpu_thread, i32 kernel_id, u64 host_op_id, u32 stream_id, u32 cubin_id, u32 mode_id,
 ...
 private:
  ...
  enum EdgeType { DATA_FLOW_EDGE_ORDER, DATA_FLOW_EDGE_READ, DATA_FLOW_EDGE_SINK }; // @Lin-Mao ?
  ...
  typedef Graph<Index, Node, EdgeIndex, Edge> DataDependencyGraph;

  struct DataDependencyTrace final : public Trace {
    Map<u64, Set<MemoryRange>> read_memory;
    Map<u64, Set<MemoryRange>> write_memory;

    DataDependencyTrace() = default;

    virtual ~DataDependencyTrace() {}
  };
 ...
 private:
  ...
  std::shared_ptr<DataDependencyTrace> _trace;

  DataDependencyGraph _graph;
...
#endif  // REDSHOW_ANALYSIS_DATA_DEPENDENCY_H
