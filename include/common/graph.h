...
class Graph {
 ...
 public:
  ...
  typename NodeMap::iterator nodes_begin() noexcept { return _nodes.begin(); }

  typename NodeMap::iterator nodes_end() noexcept { return _nodes.end(); }

  typename EdgeMap::iterator edges_begin() noexcept { return _edges.begin(); }

  typename EdgeMap::iterator edges_end() noexcept { return _edges.end(); }
  ...
  size_t incoming_edge_size(const Index &index) const {
    if (_incoming_edges.find(index) == _incoming_edges.end()) noexcept {
      return 0;
    }
    return _incoming_edges.at(index).size();
  }

  const Set<EdgeIndex> &incoming_edges(const Index &index) const noexcept { return _incoming_edges.at(index); }

  // Edge methods
  bool has_edge(const EdgeIndex &edge_index) const noexcept {
    return _edges.has(edge_index);
  }
  ...
  void add_edge(EdgeIndex &&edge_index, Args &&... edge) {
    ...
    if (inserted) {
      _incoming_edges[to].emplace(std::forward<EdgeIndex>(edge_index));
      _outgoing_edges[from].emplace(std::forward<EdgeIndex>(edge_index));
    }
  }

  void remove_edge(const EdgeIndex &edge_index) {
    _incoming_edges[to].erase(edge_index);
    _outgoing_edges[from].erase(edge_index);
